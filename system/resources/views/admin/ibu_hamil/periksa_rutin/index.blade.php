        ...
        <div class="col-12">
            <section class="card">
                <header class="card-header text-uppercase" style="height: 70px;">
                    <form action="{{ url('admin/ibu_hamil/periksa_rutin') }}" method="GET" class="form-horizontal form-label-left mb-4">
                        <div class="col-md-12">
                            <div class="input-group mb-2">
                                <input name="search" type="text" class="form-control" value="{{ request('search') }}"
                                    placeholder="Cari Berdasarkan Nama Ibu / NIK" required>
                                ...
                                @if (request('search'))
                                    <div class="input-group-append ml-1">
                                        <a href="{{ url('admin/ibu_hamil/periksa_rutin') }}" class="btn btn-secondary btn-icon-split">
        ...
        <div class="col-sm-12">
            <div class="card-box">
                <div class="button-container">
                    <a href="{{ url('/admin/ibu_hamil/periksa_rutin/create') }}" class="btn btn-primary">
                        Tambah Pemeriksaan
                    </a>
                </div><br>
                        ...
                        <tbody>
                            @forelse ($periksaRutin as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->ibu->ibu_nama ?? '-' }}</td>
                                    <td>{{ $item->ibu->ibu_nik ?? '-' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('l, d F Y') }}</td>
                                    <td>{{ $item->keluhan }}</td>
                                    <td>{{ $item->berat_badan }} kg</td>
                                    <td>{{ $item->tinggi_fundus }} cm</td>
                                    <td>
                                        <a href="{{ url('admin/ibu_hamil/periksa_rutin/show', $item->id) }}" class="btn btn-dark btn-sm">Lihat</a>
